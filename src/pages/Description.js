...
const Description = (props) => {
    ...
    const [style, setStyle] = useState(window.sessionStorage.getItem('style'));
    const [emo, setEmo] = useState('Neutral');
    const [title, setTitle] = useState(window.sessionStorage.getItem('title'));
    const [desc, setDesc] = useState(window.sessionStorage.getItem('desc'));
        ...
        fetch(serverUrl, {
          ...
          .then(response => {
            if (response.res === false) {alert('[Error] '+response.message);}
                ...
                window.sessionStorage.setItem('style', response.style);
                window.sessionStorage.setItem('desc', response.desc);
                window.sessionStorage.setItem('folder', response.folder);
                window.sessionStorage.setItem('sentences', JSON.stringify(response.sentences));
                window.sessionStorage.setItem('chars', JSON.stringify(response.chars));
                window.sessionStorage.setItem('scenes', JSON.stringify(response.scenes));
                window.sessionStorage.setItem('acts', JSON.stringify(response.acts));
                //alert('Success '+window.sessionStorage.getItem('folder'));
    ...
    return (
        <div className='input-container' style={{textAlign:'left'}} onSubmit={handleSubmit}>
            <label className='h3-light'>Please describe your experience.</label>
            ...
            <form className='desc-form' style={{marginTop:'160px', marginBottom:'120px'}} >
                ...
                <div class='row'>
                    ...
                    <label className='input-title'>Experience Title.</label>
                    ...
                    <div class='column' style={{width:'25%'}}>
                        <label className='input-title' style={{marginLeft:'60px'}}>Experience Emotion.</label>
                    ...
                    <div class='column' style={{width:'25%'}}>
                    <label className='input-title' style={{marginLeft:'60px'}}>Experience Style.</label>
                ...
                <label className='input-title' style={{marginTop:'20px'}}>Experience Description.</label>
