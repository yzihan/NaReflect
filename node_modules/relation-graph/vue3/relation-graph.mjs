...
class Ue {
  ...
  initNodesPosition() {
    ...
    t.lot.x = -(t.el.offsetWidth || t.width || 60) / 2, t.lot.y = -(t.el.offsetHeight || t.height || 60) / 2, _("root position:", t.lot.x, t.lot.y), _(
      ...
      e.fixed !== !0 && (!P.isAllowShowNode(e) || (e.x = e.lot.x || 50, e.y = e.lot.y || 50, e.lot.placed = !0));
    });
  }
...
class tt {
  ...
  analysisNodes4Didirectional(t, e, s, o, i) {
    ...
    e.forEach((c) => {
      c.lot || (c.lot = { childs: [] }), c.lot.eached = !0, c.lot.subling = l, c.lot.level = s, t.push(c);
    });
    let a = 0;
    e.forEach((c) => {
      let d = 0;
      if (i === -1) {
        ...
        c.targetFrom.forEach((u) => {
          u.lot || (u.lot = { childs: [], eached: !1 }), u.lot.eached || (P.isAllowShowNode(u) ? (u.lot.eached = !0, u.lot.parent = c, u.lot.index_of_parent = h++, c.lot.childs.push(u), r.push(u), d++) : c.lot.childs.push(u));
        });
      } else {
        let h = 0;
  ...
  placeNodesPosition(t, e, s) {
    if (t.fixed !== !0) {
      const l = this.layoutOptions.centerOffset_x || 0, r = this.layoutOptions.centerOffset_y || 0, a = t.el.offsetWidth || t.width || 60, c = t.el.offsetHeight || t.height || 60;
      t.lot.x = -a / 2 + l, t.lot.y = -c / 2 + r, this.layoutOptions.from === "top" ? t.lot.y -= this.graphOptions.viewSize.height / 2 - c - 100 : this.layoutOptions.from === "bottom" ? t.lot.y += this.graphOptions.viewSize.height / 2 - c - 200 : this.layoutOptions.from === "right" ? t.lot.x += this.graphOptions.viewSize.width / 2 - a - 100 : t.lot.x -= this.graphOptions.viewSize.width / 2 - a - 100, _("graph offset:", l, r), _("create rootNode coordinates:", t.lot.x, t.lot.y), t.x = t.lot.x + 0, t.y = t.lot.y + 0;
    } else
    ...
    t.lot.placed = !0, this.placeRelativePosition(t, s), e.forEach((l) => {
      if (l.fixed === !0) {
        l.lot.placed = !0;
        return;
      }
    ...
    if (this.layoutOptions.from === "left" || this.layoutOptions.from === "right") {
      ...
      d < i && (d = i), d > l && (d = l), this.allNodes.forEach((h) => {
        h.fixed !== !0 && h.lot.placed !== !0 && h !== t && (this.layoutOptions.from === "right" ? h.lot.x = t.lot.x - this.getLevelDistance(
          ...
          h.lot.subling.level,
          c
        ) : h.lot.x = t.lot.x + this.getLevelDistance(
          h,
          h.lot.subling.level,
          c
        ));
      }), this.allNodes.forEach((h) => {
        h.fixed !== !0 && h.lot.level !== 0 && (h.lot.y = t.lot.y + d * (e.max_strength / -2 + h.lot.strengthWithChilds_from + h.lot.strengthWithChilds / 2));
      });
    } else {
      ...
      d < i && (d = i), d > l && (d = l), this.allNodes.forEach((h) => {
        h.fixed !== !0 && h.lot.placed !== !0 && h !== t && (this.layoutOptions.from === "bottom" ? h.lot.y = t.lot.y - this.getLevelDistance(
          ...
          h.lot.subling.level,
          d
        ) : h.lot.y = t.lot.y + this.getLevelDistance(
          h,
          h.lot.subling.level,
          d
        ));
      }), this.allNodes.forEach((h) => {
        h.fixed !== !0 && h.lot.level !== 0 && (h.lot.x = t.lot.x + c * (e.max_strength / -2 + h.lot.strengthWithChilds_from + h.lot.strengthWithChilds / 2));
      });
    }
  }
...
class nt extends fe {
  ...
  placeNodes(e, s) {
    ...
    P.analysisNodes4Didirectional(
      ...
      i.x = i.lot.x + l, i.y = i.lot.y + r, i.lot.placed = !0;
    }), _("Start Auto Layout.....");
  }
  placeRelativePosition(e, s) {
    ...
    this.allNodes.forEach((a) => {
      ...
      if (a.lot.parent && a.lot.subling && a.lot.subling.level > 1) {
        const d = a.lot.parent.lot.strength_plus - a.lot.parent.lot.strength, u = (a.lot.parent.lot.strength_plus - d) / (a.lot.parent.lot.childs_size + 1) * (a.lot.index_of_parent + 1), w = P.getOvalPoint(
          e.lot.x,
          e.lot.y,
          (a.lot.subling.level - 1) * r + l,
          d + u,
          (c = a.lot.parent.lot.subling) == null ? void 0 : c.all_strength
        );
        a.lot.x = w.x, a.lot.y = w.y;
      }
    });
  }
}
...
class it extends fe {
  ...
  placeNodes(e, s) {
    ...
    let d = c * 90 / Math.PI / 2;
    d < 200 && (d = 200), d > 800 && (d = 800), this.allNodes.forEach((h, u) => {
      const w = P.getOvalPoint(
        a.x,
        a.y,
        d,
        u,
...
class ht extends dt {
  ...
  initLayouter() {
    if (this.userLayouerClass)
      ...
      ), e.layouter ? this.options.layouter = e.layouter : this.options.layouter = Me(
        e,
        this.options
      );
    } else
  ...
  removeNodeById(e) {
    ...
    for (let i = 0; i < this.graphData.links.length; i++) {
      const l = this.graphData.links[i];
...
class pt extends ft {
  ...
  findChilds(t, e) {
    e.push(t), t.targetNodes.forEach((s) => {
      e.includes(s) || this.findChilds(s, e);
    });
  }
  placeSingleNode() {
    let t = 0;
...
class mt extends _t {
  ...
  downloadAsImage(t, e = "png") {
    ...
    this.graphData.nodes.forEach((C) => {
      C.x < r && (r = C.x), C.x > c && (c = C.x + C.el.offsetWidth), C.y < a && (a = C.y), C.y > d && (d = C.y + C.el.offsetHeight);
    });
    const u = r - h, w = a - h;
    this.graphData.nodes.forEach((C) => {
      C.x = C.x - u, C.y = C.y - w;
    });
    const x = this.options.canvasOffset.x, g = this.options.canvasOffset.y;
    this.options.canvasOffset.x = h * -1, this.options.canvasOffset.y = h * -1, _("offset:", {
      _origin_offset_x: x,
      _origin_offset_y: g,
      _min_x: r,
      _min_y: a,
...
class gt extends mt {
  ...
  createLinePath(t, e, s) {
    ...
    let G = "";
    if (l === 4) {
      const I = 60 / (t.relations.length + 1) * (s + 1) - 30;
      r === "v" ? ($ = $ - (A * 33 + I), u.x = C + E + 5, u.y = B + A * 40 + I, u.rotate = 90, G = `M ${C} ${B} v${A * 33 + I} h${E + I} v${$}`) : e.reverseText === !0 ? (u.x = C + A * 10 - (A < 0 ? 30 : 0), u.y = B - 5, E = E - A * 120, G = `M ${C} ${B} h${A * 120} v${$} h${E}`) : (u.x = C + A * 50 - (A < 0 ? 30 : 0), u.y = B + $ - 5 + I, E = E - (A * 33 + I), $ = $ + A * I, G = `M ${C} ${B} h${A * 33 + I} v${$} h${E}`);
    } else if (l === 2) {
      const I = k.y > S.y ? 1 : -1, K = Math.abs(E) + Math.abs($);
      u.x = Math.round(k.x - E / K * 60 - 20), u.y = Math.round(
        k.y - $ / K * 60 - 20 * I
      );
      const ce = 1 / (t.relations.length + 1) * (s + 1) - 0.5 + 0.5;
      r === "v" ? G = `M${C},${B} c${0},${A * 30} ${E * ce},${A * -10} ${E},${$}` : G = `M${C},${B} c${A * 30},${0} ${A * -10},${$ * ce} ${E},${$}`;
    } else if (l === 6) {
      const I = k.y > S.y ? 1 : -1, K = Math.abs(E) + Math.abs($);
      u.x = Math.round(k.x - E / K * 60 - 20), u.y = Math.round(
        k.y - $ / K * 60 - 20 * I
      ), r === "v" ? G = `M${C},${B} c${0},${$ / 2} ${E},${$ / 2} ${E},${$}` : G = `M${C},${B} c${E / 2},${0} ${E / 2},${$} ${E},${$}`;
    } else if (l === 3) {
      u.x = k.x - A * 63, u.y = k.y + 3;
      const I = 1 / (t.relations.length + 1) * (s + 1) - 0.5 + 0.5;
      r === "v" ? G = `M${C},${B} c${0},${$ * I} ${0},${0} ${E},${$}` : G = `M${C},${B} c${A * 30},${0} ${A * -10},${$ * I} ${E},${$}`;
    } else if (l === 5) {
      u.x = k.x - A * 63, u.y = k.y + 3;
      const I = 1 / (t.relations.length + 1) * (s + 1) - 0.5 + 0.5;
      r === "v" ? G = `M${C},${B} c${0},${0} ${0},${$ * I} ${E},${$}` : G = `M${C},${B} c${0},${0} ${E * I},${0} ${E},${$}`;
    } else {
      const I = P.getAngleType(E, $);
      u.rotate = P.getTextAngle(C, B, Q, ae);
      const K = I === 2 || I === 4 ? -1 : 1, ce = ($ === 0 ? -50 : Math.sin(Math.atan(E / $)) * 10 / Math.sin(90)) * K, ze = E === 0 ? -50 : Math.sin(Math.atan($ / E)) * 10 / Math.sin(90);
      u.x = Math.round(S.x + E / 2 - ce), u.y = Math.round(S.y + $ / 2 - ze), Number.isNaN(u.rotate) && (u.rotate = 0), G = `M ${C} ${B} L ${Q} ${ae}`;
    }
...
const At = /* @__PURE__ */ q(Bt, [["__scopeId", "data-v-c6092e44"]]), It = ["id", "d", "stroke", "marker-start", "marker-end"], Dt = /* @__PURE__ */ T({
  __name: "RGLinePath",
  props: {
    link: null,
    relation: null,
  ...
  setup(n) {
    const t = W(), e = X(j).value, s = () => {
      ...
      let d = a.deltaY;
      d === void 0 && (d = a.wheelDelta);
      const h = -1;
      d > 0 ? e.zoom(5 * h, c) : e.zoom(-5 * h, c);
    }, i = () => {
    }, l = (a) => {
      ...
      if (ie) {
        ...
        c = (u, w, x, g, m) => {
          ...
          if (y && y.length > 1) {
            ...
            const S = z.clientX, k = z.clientY, H = x.x + (S - g.x), C = x.y + (k - g.y);
            e.setCanvasOffset(H, C);
          }
        };
      }
      ge.startDrag(
    ...
    return ue(() => {
      ...
      style: F({
        width: "100%",
        height: "100%",
        "background-image": "url(" + f(e).options.backgrounImage + ")"
      }),
      class: D([[
        f(e).options.layoutClassName,
        f(e).options.backgrounImageNoRepeat ? "rel-map-background-norepeat" : ""
      ], "rel-map"]),
      ...
      p("div", {
        ...
        O(a.$slots, "canvas-plug", {}, void 0, !0),
        p("div", {
          ref_key: "rgCanvas$",
          ref: t,
          class: "rel-linediv",
          ...
          (v(), b("svg", {
            ...
            p("defs", null, [
              p("linearGradient", {
                id: f(e).options.instanceId + "-lineStyle",
                x1: "1",
                y1: "0",
                x2: "0",
                y2: "0"
              ...
              p("marker", {
                id: f(e).options.instanceId + "-arrow-default",
                markerWidth: f(e).options.defaultLineMarker.markerWidth,
                markerHeight: f(e).options.defaultLineMarker.markerHeight,
                refX: f(e).options.defaultLineMarker.refX,
                refY: f(e).options.defaultLineMarker.refY,
                "marker-units": "strokeWidth",
                orient: "auto",
                viewBox: "0 0 12 12"
              }, [
                p("path", {
                  style: F({ fill: f(e).options.defaultLineColor }),
                  d: f(e).options.defaultLineMarker.data
                }, null, 12, Rt)
              ], 8, Vt),
              p("marker", {
                id: f(e).options.instanceId + "-start-arrow-default",
                markerWidth: f(e).options.defaultLineMarker.markerWidth,
                markerHeight: f(e).options.defaultLineMarker.markerHeight,
                refX: f(e).options.defaultLineMarker.refX,
                refY: f(e).options.defaultLineMarker.refY,
              ...
              p("marker", {
                id: f(e).options.instanceId + "-arrow-checked",
                ...
                p("path", {
                  style: F({ fill: f(e).options.checkedLineColor }),
                  d: "M2,2 L10,6 L2,10 L6,6 L2,2"
                }, null, 4)
              ], 8, jt),
              p("marker", {
                id: f(e).options.instanceId + "-start-arrow-checked",
                markerUnits: "strokeWidth",
                markerWidth: "12",
                markerHeight: "12",
                viewBox: "0 0 12 12",
              ...
              (v(!0), b(R, null, Z(f(e).allLineColors, (d) => (v(), b("marker", {
                ...
                p("path", {
                  fill: f(e).options.defaultLineMarker.color || d.color,
                  d: f(e).options.defaultLineMarker.data
                }, null, 8, Yt)
              ], 8, Zt))), 128)),
              (v(!0), b(R, null, Z(f(e).allLineColors, (d) => (v(), b("marker", {
        ...
        p("div", qt, [
          (v(!0), b(R, null, Z(f(e).graphData.nodes, (d) => (v(), se(kt, {
            ...
            node: re(({ node: h }) => [
              O(a.$slots, "node", { node: h }, void 0, !0)
            ]),
            _: 2
          }, 1032, ["node-props"]))), 128))
        ])
  ...
  setup(n) {
    ...
    return (r, a) => (v(), b("div", null, [
      ...
      e.value ? (v(), b("div", {
        ...
        p("div", eo, [
          Be(" debug log status: " + ne(f(t).options.debug), 1),
          p("button", { onClick: l }, ne(f(t).options.debug ? "disable" : "enable") + " debug log ", 1)
        ])
      ])) : N("", !0)
    ]));
  }
...
const oo = /* @__PURE__ */ q(to, [["__scopeId", "data-v-88390d64"]]), no = {
  ...
  setup(n) {
    const t = X(j).value, e = W(100), s = W(0), o = {
      ...
      let r = 9999, a = -9999, c = 9999, d = -9999;
...
const io = /* @__PURE__ */ q(so, [["__scopeId", "data-v-1b3b937f"]]), M = (n) => (be("data-v-c51977d9"), n = n(), Ce(), n), ao = /* @__PURE__ */ M(() => /* @__PURE__ */ p("svg", {
  ...
  /* @__PURE__ */ p("use", { "xlink:href": "#icon-lianjieliu" })
], -1)), Io = /* @__PURE__ */ M(() => /* @__PURE__ */ p("span", { class: "c-mb-text" }, "\u9C7C\u5C3E", -1)), Do = [
  Ao,
  Io
...
], -1)), Ro = /* @__PURE__ */ M(() => /* @__PURE__ */ p("span", { class: "c-mb-text" }, "\u8FDE\u63A5\u70B9", -1)), Oo = {
  class: "c-mb-child-panel",
  style: { width: "206px", "margin-left": "-205px" }
}, To = /* @__PURE__ */ M(() => /* @__PURE__ */ p("svg", {
  class: "rg-icon",
  "aria-hidden": "true"
}, [
  /* @__PURE__ */ p("use", { "xlink:href": "#icon-lianjie_connecting5" })
  ...
  setup(n) {
    ...
    const i = () => {
      ...
      _("change layout:", c), rt(c, t.options), i();
    }, r = () => {
      t.startAutoLayout();
    }, a = () => {
      s.value++, setTimeout(() => {
    ...
    return (c, d) => (v(), b("div", {
      ...
      p("div", {
        ...
        onClick: d[0] || (d[0] = (h) => f(t).fullscreen())
      }, [
        ao,
      ...
      f(t).options.layouts.length > 1 ? (v(), b("div", _o, [
        ...
        p("div", {
          style: F({
            width: f(t).options.layouts.length * 70 + 6 + "px",
            "margin-left": f(t).options.layouts.length * -70 - 5 + "px"
          }),
          class: "c-mb-child-panel"
        }, [
      ...
      f(t).options.allowSwitchLineShape ? (v(), b("div", wo, [
        ...
        p("div", ko, [
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultLineShape === 1
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[3] || (d[3] = (h) => f(t).setDefaultLineShape(1))
          }, So, 2),
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultLineShape === 2
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[4] || (d[4] = (h) => f(t).setDefaultLineShape(2))
          }, Eo, 2),
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultLineShape === 6
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[5] || (d[5] = (h) => f(t).setDefaultLineShape(6))
          }, Bo, 2),
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultLineShape === 5
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[6] || (d[6] = (h) => f(t).setDefaultLineShape(5))
          }, Do, 2),
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultLineShape === 4
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[7] || (d[7] = (h) => f(t).setDefaultLineShape(4))
          }, Wo, 2)
        ])
      ])) : N("", !0),
      f(t).options.allowSwitchJunctionPoint ? (v(), b("div", Go, [
        ...
        p("div", Oo, [
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultJunctionPoint === "border"
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[8] || (d[8] = (h) => f(t).setDefaultJunctionPoint("border"))
          }, Jo, 2),
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultJunctionPoint === "ltrb"
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[9] || (d[9] = (h) => f(t).setDefaultJunctionPoint("ltrb"))
          }, Xo, 2),
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultJunctionPoint === "tb"
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[10] || (d[10] = (h) => f(t).setDefaultJunctionPoint("tb"))
          }, Qo, 2),
          p("div", {
            class: D([{
              "c-mb-button-on": f(t).options.defaultJunctionPoint === "lr"
            }, "c-mb-button c-mb-button-c"]),
            style: { width: "50px" },
            onClick: d[11] || (d[11] = (h) => f(t).setDefaultJunctionPoint("lr"))
          }, tn, 2)
        ])
      ])) : N("", !0),
      ...
      f(t).options.allowShowDownloadButton ? (v(), b("div", pn, [
        ...
        p("div", {
          ...
          p("div", {
            class: "c-mb-button c-mb-button-c",
            style: { width: "50px" },
            onClick: d[13] || (d[13] = (h) => f(t).downloadAsImage("jpg"))
          }, bn)
        ], 4)
      ])) : N("", !0)
    ], 4));
...
const kn = /* @__PURE__ */ q(Cn, [["__scopeId", "data-v-c51977d9"]]), Mn = /* @__PURE__ */ T({
  __name: "index",
  props: {
    options: null,
